var DatatablesBasicBasic = function() {
	...
	var initTable1 = function() {
...

				{
					...
					render: function(data, type, full, meta) {
						return `
                         ...
                         <a href="appGrafica/alterar_cadastro.php?id_grafica=`+full[0]+`"" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Visualizar Cadastro">
                          <i class="la la-edit"></i>
                        </a>
						<a id="excluir"class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Deletar" data-grafica="`+full[0]+`" >
		...
		table.on('click', '#excluir', function() {
			var id_grafica = $(this).data("grafica");
			swal.fire({
	            title: 'Tem certeza?',
	            text: "Desejar excluir a Gráfica?",
	            type: 'warning',
	            ...
	            if (result.value) {
					$.ajax({
				        url: 'appGrafica/excluir_grafica.php'
				        , type: 'post'
