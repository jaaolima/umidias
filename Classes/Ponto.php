    ...
    class Ponto{
		...
		public function listarMeusPontosAlugadosParceiro($id_parceiro)
		{
			try{
				...
				$select = "SELECT p.id_ponto, nu_valor, t.id_midia, ds_local, t.ds_tipo, f.ds_foto, ds_descricao,  ds_observacao, u.ds_nome, a.dt_inicial, a.dt_final
							FROM rl_alugado a
							right join tb_ponto p on a.id_ponto=p.id_ponto
							right join rl_ponto_foto f on p.id_ponto=f.id_ponto
							inner join tb_tipo_midia t on p.id_midia=t.id_midia
		...
		function BuscarDadosAlugado($id_alugado)
		{
			try{
				...
				$select = "SELECT 
							a.id_ponto, ds_descricao, ds_latitude, ds_longitude, p.nu_valor, ds_tipo, ds_observacao, ds_local, ds_tamanho,  p.id_midia, a.ds_material, id_periodo, id_parceiro, dt_inicial, dt_final, nu_valor_alugado
						FROM rl_alugado a
						right join tb_ponto p on a.id_ponto=p.id_ponto
						inner join tb_tipo_midia t on p.id_midia=t.id_midia
		...
		function BuscarAlugadoPonto($id_ponto)
		{
			try{
				...
				$select = "SELECT a.id_alugado, a.id_ponto, ds_descricao, ds_latitude, ds_longitude, nu_valor, ds_tipo, ds_observacao, ds_local, ds_tamanho,  p.id_midia, a.id_material, id_periodo, p.id_parceiro, dt_inicial, dt_final, ds_nome
						FROM rl_alugado a
						right join tb_ponto p on a.id_ponto=p.id_ponto
						inner join tb_tipo_midia t on p.id_midia=t.id_midia
						inner join tb_usuario u on a.id_usuario=u.id_usuario
