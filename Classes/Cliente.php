    ...
    class Cliente{
		...
		function buscarCarrinho($id_usuario)
		{
			try{
				...
				$select = "SELECT id_carrinho, c.id_ponto, ds_descricao, ds_latitude, ds_longitude, f.ds_foto, nu_valor, t.ds_tipo, ds_observacao, ds_local, ds_tamanho, ds_material, p.id_midia, p.id_material, id_periodo, id_parceiro
						FROM rl_carrinho c 
						right join tb_ponto p on c.id_ponto = p.id_ponto 
						inner join tb_tipo_midia t on p.id_midia=t.id_midia 
						inner join tb_tipo_midia m on p.id_material=m.id_material 
